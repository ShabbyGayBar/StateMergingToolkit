    ...
    if isinstance(dic, list):
        ...
        if len(dic) == 1:
            if isinstance(dic[0], dict):
                if "country" in dic[0].keys() or "owned_provinces" in dic[0].keys() :
                    return dic
            elif isinstance(dic[0], str):
                return dic
            return dic[0]
...
states = remove_idle_element(states)
...
with open(mod_dir["States"]+"/all_states.json", "w", encoding="utf-8") as file:
    json.dump(states["STATES"], file, ensure_ascii=False, indent=4)
